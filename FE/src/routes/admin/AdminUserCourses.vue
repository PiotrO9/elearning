...
async function fetchUserData() {
    const response = await httpClient.get(`/users`).then((res) => res.data)
    const userExists = response.data.items.some((user: { id: string }) => user.id === userId.value)
    if (!userExists) {
        router.push('/admin/users')
    }
    userData.value =
        response.data.items.find((user: { id: string }) => user.id === userId.value) || null
...
onMounted(async () => {
    await Promise.all([fetchUserData(), fetchCourses(), fetchEnrolledCourses()])
})
</script>
            ...
            <div class="mb-8">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <AdminTableHeader
                        ...
                        :description="`Przypisz lub usuń kursy dla użytkownika : ${userData?.username || userData?.email}`"
                    />
                </div>
            </div>
                ...
                >
                    <div class="flex items-center justify-between">
                        <div class="flex-1">
                            ...
                            <p v-if="course.summary" class="text-sm text-gray-600 line-clamp-2">
                                {{ course.summary }}
                            </p>
